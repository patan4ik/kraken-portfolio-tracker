...
def test_compute_trends_adds_columns(tmp_path, monkeypatch, patch_api_and_keys):
    ...
    prev = pd.DataFrame({
        "Asset": ["ETH", "SOL", "ZEUR"],
        "Value (EUR)": [2000.0, 200.0, 100.0]
    })
    prev.to_csv(os.path.join(str(balances_dir), f"balance_{prev_date}.csv"), index=False, encoding="utf-8")
    ...
    assert float(out.loc[out["Asset"] == "ETH", "Portfolio Trend Avg"].values[0]) == 200.0
